...
if __name__ == "__main__":
    ...
    for veh in [ego,other]:
        ...
        for dx,dv in zip([x[0] for x in spec], [x[1] for x in spec]):
            label = "dx-{},dv-{}".format(dx,dv)
            dest_state = dict(veh_state)
            dest_state["position"] = tuple([dest_state["position"][0]+dx,dest_state["position"][1]])
            dest_state["velocity"] += dv
            dest_state["parametrised_acceleration"] = (0,0) #parametrised acceleration is introduced to handle acceleration constraints
            traj = Trajectory(veh_state,dest_state,T,label)
            vehicle_traj_list.append(traj)

        vehicle_traj_dict[veh] = list(vehicle_traj_list)
    ...
    reward_grid = np.empty((len(vehicle_traj_specs[other]),len(vehicle_traj_specs[ego]),2))
    for i,other_traj in enumerate(vehicle_traj_dict[other]):
        for j,ego_traj in enumerate(vehicle_traj_dict[ego]):
            if checkForCrash(ego_traj,other_traj,dt,safety_params): reward_grid[i,j] = ((-np.inf,-np.inf))
            else:
                ego_r = vehicle_traj_rewards[ego][j]
                other_r = vehicle_traj_rewards[other][i]
                reward_grid[i,j] = (((1-alt_other)*other_r+alt_other*ego_r,(1-alt_ego)*ego_r+alt_ego*other_r))

    print("CONFUSION MATRIX")
    for i in range(len(vehicle_traj_dict[other])):
        print("{}".format("\t".join([str((reward_grid[i,j,0],reward_grid[i,j,1])) for j in range(len(vehicle_traj_dict[ego]))])))


    #Stackelberg Game, assumes Other/Non-Ego/In-Lane agent is initial decider
    max_other_r,max_ego_r = None,None
    max_other_index,max_ego_index = None,None
    for i in range(reward_grid.shape[0]):
        if max_other_r is None or max(reward_grid[i,:,0])>max_other_r:
            max_other_r = max(reward_grid[i,:,0])
            max_other_index = i
            max_ego_r = max(reward_grid[i,:,1])
            max_ego_index = np.argmax(reward_grid[i,:,1])
            
    print("\n")
    print(f"alpha_Other: {alt_other}\talpha_E: {alt_ego}")
    print(f"Other chooses: {max_other_index} and Ego chooses: {max_ego_index}")
    ###################################
    import pdb
    pdb.set_trace()
    ###########################################
