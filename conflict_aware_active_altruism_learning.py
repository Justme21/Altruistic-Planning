...
def expectedValue(matrix_row,intersection_ranges,r_alpha,c_alpha_distr):
    ...
    e_reward = 0
    #Prob is the probability of C choosing action i, if the given row of the game matrix were chosen by R,
    ...
    ######################################
    #NOTE: Here we are hard-coding that conflict exists for the purposes of getting results
    # What should be done is check if conflict exists in the reward matrix for each possible distr value
    
    conflict_range = [0,0.5]
    p_conflict = 0
    conflict_preferred_index = 1
    conflict_reward = computeReward(matrix_row[conflict_preferred_index],r_alpha,0)
    for alpha_range in c_alpha_distr:
        p_conflict += c_alpha_distr[alpha_range]*(min(alpha_range[1],max(alpha_range[0],conflict_range[1])) - max(alpha_range[0],min(alpha_range[1],conflict_range[0])))/(alpha_range[1]-alpha_range[0])
        
    #e_reward = (1-p_conflict)*e_reward + p_conflict*conflict_reward
...
if __name__ == '__main__':
    ...
    #Values used in MPhil Active Learning Experiment
    #r_values = [[3,-5],[-1,1],[-1,2]]
    #c_values = [[0,7],[2,1],[2,2]]

    #Values used in Information Sufficiency MPhil experiment
    #r_values = [[5,-2],[1,0]]
    #c_values = [[-4,1],[-4,1]]
    ...
    r_values = [[1,-1],[-1,0],[1,0]]
    c_values = [[0,-1],[-1,1],[0,1]]
    ...
    #Perform Expected Value calculation out here since computing the turning point probability uses the reward values
    #augmented_expected_value = rewardMatrixExpectedValues(augmented_reward_matrix,preferred_actions,r_alpha,c_alpha_distr)
   ...
   ########################################
    ...
    #optimizer = NestedMaximizer(zh+zr, uh, zh+zr, ur,bounds=bounds)
    optimizer = NestedMaximizer(zh, uh, zr, ur,bounds=bounds)

    #optimizer = Maximizer(zh+zr, uh+ur,bounds=bounds)
    #optimizer = Maximizer(zh+zr, uh+ur, method='CG',bounds=bounds)
    #optimizer.maximize()
    ...
    for n in range(int(6*N)):
        ...
        expected_values = [round(x,3) for x in expected_values]
        ...
        #    #action_distr = [np.clip(np.exp(cost_scale_coef*hcost_eval(xh_temp,uh[i].get_value(),w,xr_temp,ur[i].get_value())),1e-8,1e8) for w in h_types]
    ...
    filename = "CONFLICTUNAWARE{}".format(updateMatrix.__name__)
