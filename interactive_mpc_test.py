...
obs_y = 15
obs_v = 15
obs_heading = math.degrees(math.pi/2)

obs_init_state = {"position":(obs_x,obs_y),"velocity":obs_v,"heading":obs_heading,"parametrised_acceleration":(0,0)}

label = "dx-{},dv-{}".format(obs_dx,obs_dv)
obs_dest_state = dict(obs_init_state)
obs_dest_state["position"] = tuple([obs_dest_state["position"][0]+obs_dx,obs_dest_state["position"][1]])
obs_dest_state["velocity"] += obs_dv
obs_dest_state["parametrised_acceleration"] = (0,0) #parametrised acceleration is introduced to handle acceleration constraints
obs_trajectory = Trajectory(obs_init_state,obs_dest_state,T,label)

obs_pos_list = obs_trajectory.completePositionList(dt)
obs_vel_list = obs_trajectory.completeVelocityList(dt)
obs_heading_list = obs_trajectory.completeHeadingList(dt)

pdb.set_trace()

obs_traj = np.array([(p[0],p[1],v,h) for p,v,h in zip(obs_pos_list,obs_vel_list,obs_heading_list)]).T
...
bounds = [0,2*lane_width,0,speed_limit,0,math.pi,-3,3,-math.pi/9,math.pi/9]
...
obs_x = opti.parameter(4,N+1)
opti.set_value(obs_x,obs_traj)
...
progress_x = (x[0,-1]-goal[0])**2 # final distance from goal x-position
progress_y = (x[1,-1]-goal[1])**2 # final distance from goal y-position
desired_velocities = (x[2,-1]-goal[2])**2
progress_heading = (x[3,-1]-goal[3])**2 # final distance from goal y-position

comfort_accel = sumsqr(u[0,:])
comfort_yaw = sumsqr(u[1,:])
...
G = Function('G',[x,u],[g_out],['x','u'],['G-out'])
###########################################

#opti.set_value(w_temp,[1,0,0,0])
#opti.minimize(sumsqr((x[:,-1]-goal)*w_temp)) # Distance to destination
opti.minimize(G(x,u))
#This can also be done with functional programming (mapaccum)
for k in range(N):
    opti.subject_to(x[:,k+1]==F(x[:,k],u[:,k]))
    #Avoid crash constraint
    opti.subject_to(((x[0,k+1]-obs_x[0,k+1])**2)/(safety_params[0]**2) + ((x[1,k+1]-obs_x[1,k+1])**2)/(safety_params[1]**2)>=1)
